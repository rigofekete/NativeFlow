...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
        ...
        // TODO:  Older case with relevant comments to be added to the fully implemented one 
        // case WM_DRAWITEM:
        // {
        //     DRAWITEMSTRUCT* dis = (DRAWITEMSTRUCT*)lParam;
        ...
        //     // Only draw if NOT editing
        //     if(!task.isEditing) 
        //     {
        //         // Calculate Y position taking into account previous tasks' heights
        //         int yPos = 0;
        //         for(size_t i = 0; i < dis->itemID; i++) 
        //         {
        //             const TaskRect& prevTask = CustomListBoxState.tasks[i];
        //             int prevHeight = prevTask.bounds.bottom - prevTask.bounds.top;
        //             yPos += prevHeight + taskYSpacing;
        //         }


        //         // Set initial rect position with proper spacing
        //         rcItem.top = yPos;
        //         rcItem.left += taskXPadding;
        ...
        //         // Set width according to text width or max width 
        //         // Values with slight adjustments to better fit the Edit Control and prevent List box text       remnants/artifacts

        //         // This is now a static global since it is fixed value 
        //         // int maxTaskWidth = 210 + (taskXPadding * 3);
        ...
        //         // int textHeight = testRect.bottom - testRect.top;
        //         // int totalHeight = max(textHeight + (2 * taskYSpacing), taskHeight);
        //         // rcItem.bottom = rcItem.top + totalHeight;

        //         // Calculate wrapped height
        //         RECT testRect = rcItem;
        //         testRect.left += taskXPadding;   // Account for padding in width calculation
        //         testRect.right -= taskXPadding;  // This affects where text wraps

        //         // The purpose of adding 1000 to testRect.top is to create a temporary rectangle (testRect) with a large bottom value.
        //         // This is done to calculate the wrapped height of the text without actually drawing it on the screen.
        //         // By setting testRect.bottom to a large value (testRect.top + 1000), it ensures that the text will have enough space to wrap within the rectangle's width.
        //         testRect.bottom = testRect.top + 1000;

        //         HFONT hFont = (HFONT)GetWindowLongPtr(hwnd, GWLP_USERDATA);
        //         SelectObject(hdc, hFont);

        //         // Get text metrics for precise height calculation
        //         TEXTMETRIC tm;
        //         GetTextMetrics(hdc, &tm);
        ...
        //         int textHeight = testRect.bottom - testRect.top;
        //         // Add additional padding for descenders (like 'g', 'y', 'j', etc)
        //         // Descenders are the parts of lowercase letters that extend below the baseline, such as the tails of 'g', 'y', 'j', 'p', 'q', etc.
        //         // When calculating the height of the text using DrawText with the DT_CALCRECT flag, it only considers the height from the top of the tallest character to the baseline.
        //         // However, to ensure proper spacing and avoid clipping the descenders, additional padding needs to be added to the calculated height.
        //         // The GetTextMetrics function is used to retrieve the text metrics of the selected font, which includes information about the descenders.
        //         // The tmDescent member of the TEXTMETRIC structure represents the height of the descenders.
        //         // By adding tm.tmDescent to the calculated textHeight, it ensures that there is enough space below the baseline to accommodate the descenders.
        //         // This additional padding prevents the descenders from being clipped or overlapping with the content below the text.
        //         textHeight += tm.tmDescent;
        ...
        //           // Clear the area with black before paiting new rect/hover bounds and text  
        //         RECT clearRect = dis->rcItem;
        //         HBRUSH blackBrush = CreateSolidBrush(RGB(0, 0, 0));
        //         FillRect(dis->hDC, &clearRect, blackBrush);
        //         DeleteObject(blackBrush);
        ...
        //             // Always draw the full text in white first
        //             SetTextColor(hdc, RGB(255,255,255));
        ...
        //             // Then overlay the blue animated portion if needed
        //             if(task.blueCharCount > 0)
        //             {
        //                 if(task.blueCharCount == task.text.size())
        //                 {
        //                     // If all text should be blue, draw it all
        //                     SetTextColor(hdc, RGB(51,173,255));
        //                     DrawText(hdc, task.text.c_str(), -1, &textRect,
        //                             DT_LEFT | DT_WORDBREAK);
        //                 }
        //                 else
        ...
        //                     // IMPORTANT:  find_first_of()  function looks for any of the passed character args, from the task.blueCharCount position arg ONWARDS 
        //                     size_t wordEnd = task.text.find_first_of(L" \t\r\n", task.blueCharCount);
        //                     if(wordEnd == std::wstring::npos) 
        //                     {
        //                         wordEnd = task.text.length();
        //                     }
                            
        //                     // If we're in the middle of a word, wait until we can color the whole word
        //                     // IMPORTANT:  find_last_of()  function looks for any of the passed character args, from the task.blueCharCount -1 position arg BACKWARDS
        //                     size_t wordStart = task.text.find_last_of(L" \t\r\n", task.blueCharCount - 1);
        //                     wordStart = (wordStart == std::wstring::npos) ? 0 : wordStart + 1;
                            
        //                     std::wstring blueText;
        //                     if(task.blueCharCount >= wordEnd) 
        //                     {
        //                         // If we've reached the end of the word, color up to there
        //                         blueText = task.text.substr(0, wordEnd);
        //                     } 
        //                     else if(task.blueCharCount < wordStart) 
        //                     {
        //                         // If we haven't started the word yet, color up to the word start
        //                         blueText = task.text.substr(0, wordStart);
        //                     } 
        //                     else if(task.blueCharCount >= wordStart) 
        //                     {
        //                         // We're in the middle of a word, wait
        //                         blueText = task.text.substr(0, wordStart);
        //                     }

        //                     // Draw the blue portion on top
        //                     SetTextColor(hdc, RGB(0,122,204));
        //                     DrawText(hdc, blueText.c_str(), -1, &textRect,
        //                             DT_LEFT | DT_WORDBREAK);
        //                 }
        ...
        //             else
        //             {
        //                 // Draw normal white text when no animation is active 
        //                 SetTextColor(hdc, RGB(255,255,255));
        //                 DrawText(hdc, task.text.c_str(), -1, &textRect,
        //                         DT_LEFT | DT_WORDBREAK);
        //             }
